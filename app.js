...
app.put('/updateotp', (req, res) => {
    ...
    if (!req.body.hasOwnProperty('UserName') && !req.body.hasOwnProperty('Otp') ) {
        res.status(400).send({ "Status": 400, "Info": "Bad request" })
    } else {   
        
        connection.query(`update registrationdata set Otp=? where UserName=?`,
        [req.body.Otp,req.body.UserName], (err, row) => {
...
<div id='card' class="animated fadeIn">
  ...
  <div id='lower-side'>
    <p id='message'>
       ${req.body.Otp}
       your Otp has successfully generated.
    </p>
    <a href="https://www.google.com/" id="contBtn">Continue To LogIn</a>
  </div>
</div>
    ...
    <!-- Analytics -->
                ...
                transporter.sendMail(mail, (err, info) => {
                    ...
                    if (err) {
                        ...
                        res.status(200).send({ "status" : 200, 'Info': 'Otp  Successfully generated to mail ' })
                    }
...
app.put('/updatepassword', (req, res) => {
    if (!req.body.hasOwnProperty('Password') && !req.body.hasOwnProperty('ConfirmPassword') && !req.body.hasOwnProperty('Otp') && !req.body.hasOwnProperty('UserName')) {
        res.status(400).send({ "Status": 400, "Info": "Bad request" })
    } else {
      
        connection.query(`update registrationdata set Password=?, ConfirmPassword=? where Otp=? AND UserName=?`,
            [req.body.Password,req.body.ConfirmPassword,req.body.Otp,req.body.UserName], (err, data) => {
                if (!err) {
